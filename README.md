...
A focused, GTD-inspired task manager with a local-first stack and an opinionated **Briefing Room** UI. Think of it as a daily editorial: one card that summarises today's work, a clean Today list, an Inbox that's actually short, and a Friday review you'll do because it doesn't feel like data entry.
...
- **GTD flows** — Capture (`⌘N` from anywhere) → Inbox → process to Today / Next / Waiting / Someday / Done. Weekly Review is a real ritual with live wins, project health, and stale-item triage.
...
- **Keyboard-first** — `⌘N` to capture, `j/k` to navigate the inbox, `⏎` to send-to-next, `d` to delete, inline-edit-on-click for titles and descriptions.
...
| Hotkeys | **react-hotkeys-hook** | `⌘N`, `j/k`, `⏎`, `d` |
